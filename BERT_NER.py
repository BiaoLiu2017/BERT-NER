...
class InputExample(object):
  ...
  def __init__(self, guid, text, feature1=None, feature2=None, feature3=None, label=None):
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["[PAD]","ATTRIBUTIONCOMP", "BRANDCOMP", "Group_COMP", "KEYWORD_COMP", "Occasion_COMP", "Other","X","[CLS]","[SEP]"]
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            feature2 = tokenization.convert_to_unicode(line[2])
            feature3 = tokenization.convert_to_unicode(line[3])
            labels = tokenization.convert_to_unicode(line[-1])
            examples.append(InputExample(guid=guid, text=texts, feature1=feature1, feature2=feature2,
                                         feature3=feature3, label=labels))
        return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    for (i,label) in enumerate(label_list):
        label_map[label] = i
    if not os.path.exists(FLAGS.middle_output+"/label2id.pkl"):
        with open(FLAGS.middle_output+"/label2id.pkl",'wb') as w:
            pickle.dump(label_map,w)
    textlist = example.text.split(' ')
    labellist = example.label.split(' ')
    feature1list = example.feature1.split(' ')
    feature2list = example.feature2.split(' ')
    feature3list = example.feature3.split(' ')
    tokens = []
    labels = []
    feature1 = []
    feature2 = []
    feature3 = []
    for i,(word,label,fea1,fea2,fea3) in enumerate(zip(textlist,labellist,feature1list,feature2list,feature3list)):
        token = tokenizer.tokenize(word)
        tokens.extend(token)
        for i,_ in enumerate(token):
            feature1.append(int(fea1))
            feature2.append(int(fea2))
            feature3.append(int(fea3))
            if i==0:
                labels.append(label)
            else:
                labels.append("X")
    ...
    if len(tokens) >= max_seq_length - 1:
        ...
        feature1 = feature1[0:(max_seq_length - 1)]
        feature2 = feature2[0:(max_seq_length - 1)]
        feature3 = feature3[0:(max_seq_length - 1)]
    ntokens = []
    segment_ids = []
    label_ids = []
    ntokens.append("[CLS]")
    ...
    for i, token in enumerate(tokens):
        ...
        feature1_list.append(feature1[i])
        feature2_list.append(feature2[i])
        feature3_list.append(feature3[i])
    ...
    while len(input_ids) < max_seq_length:
        ...
        segment_ids.append(0)
        label_ids.append(0)
        feature1_list.append(0)
        feature2_list.append(0)
        feature3_list.append(0)
        ntokens.append("[PAD]")
    assert len(input_ids) == max_seq_length
    assert len(mask) == max_seq_length
    assert len(segment_ids) == max_seq_length
    assert len(label_ids) == max_seq_length
    assert len(feature1_list) == max_seq_length
    assert len(feature2_list) == max_seq_length
    assert len(feature3_list) == max_seq_length
    ...
    if ex_index < 3:
        ...
        logging.info("feature1_list: %s" % " ".join([str(x) for x in feature1_list]))
        logging.info("feature2_list: %s" % " ".join([str(x) for x in feature2_list]))
        logging.info("feature3_list: %s" % " ".join([str(x) for x in feature3_list]))
...
def filed_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file,mode=None):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["feature1"] = create_int_feature(feature.feature1)
        features["feature2"] = create_int_feature(feature.feature2)
        features["feature3"] = create_int_feature(feature.feature3)
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    name_to_features = {
        ...
        "feature1": tf.FixedLenFeature([seq_length], tf.int64),
        "feature2": tf.FixedLenFeature([seq_length], tf.int64),
        "feature3": tf.FixedLenFeature([seq_length], tf.int64),
...
def create_model(bert_config, is_training, input_ids, mask,
                 segment_ids, labels, num_labels, use_one_hot_embeddings, feature1=None, feature2=None, feature3=None):
    ...
    feature1 = tf.one_hot(feature1, depth=11, dtype=tf.float32)#64x64x11
    feature1 = tf.cast(feature1, dtype=tf.float32)
    feature2 = tf.one_hot(feature2, depth=5, dtype=tf.float32)  # 64x64x5
    feature2 = tf.cast(feature2, dtype=tf.float32)
    feature3 = tf.one_hot(feature3, depth=5, dtype=tf.float32)  # 64x64x5
    feature3 = tf.cast(feature3, dtype=tf.float32)
    feature_concat = tf.concat([output_layer, feature1, feature2, feature3], -1)#64x64x789
...
def main(_):
    ...
    run_config = tf.contrib.tpu.RunConfig(
        ...
        tpu_config=tf.contrib.tpu.TPUConfig(
            ...
            per_host_input_for_training=is_per_host)).replace(session_config=session_config)
    train_examples = None
    ...
    batch_tokens_train, batch_labels_train = filed_based_convert_examples_to_features(
    ...
    batch_tokens_dev, batch_labels_dev = filed_based_convert_examples_to_features(
    ...
    if FLAGS.do_predict:
        ...
        logging.info("Test data:  Num examples = %d", len(predict_examples))
        ...
        logging.info("Train data:    Num examples = %d", len(train_examples))
        logging.info("  Batch size = %d", FLAGS.predict_batch_size)
        predict_input_fn = file_based_input_fn_builder(
            input_file=train_file,
            seq_length=FLAGS.max_seq_length,
        ...
        Writer(output_predict_file, result, batch_tokens_train, batch_labels_train, id2label)

        logging.info("Dev data:    Num examples = %d", len(eval_examples))
        logging.info("  Batch size = %d", FLAGS.predict_batch_size)
        predict_input_fn = file_based_input_fn_builder(
            input_file=eval_file,
            seq_length=FLAGS.max_seq_length,
        ...
        Writer(output_predict_file, result, batch_tokens_dev, batch_labels_dev, id2label)
